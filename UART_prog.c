...
static u8 UART_ChannelIndex1 = 0;
static u8 UART_ChannelIndex2 = 0;
    ...
    if ( UART_Semaphore == UART_SemaphoreAvailable )
    {
        for (LoopIndex = 0 ; LoopIndex<NUMBER_OF_MODULES ; LoopIndex ++ )
        {
            ...
            case UART_SUSPENDED_State:
                ...
                {
                    UART_ChannelIndex1 =  LoopIndex;  /* in order to make the index in the info and config arrays of the UART known by the ISR instead searching again */
                ...
                {
                    UART_ChannelIndex2 =  LoopIndex;  /* in order to make the index in the info and config arrays of the UART known by the ISR instead searching again */
                    /* if the receive fifo is not empty and the Remaining receive bytes is less than the trigger length then no interrupt will occur then this should be handled by the main */
...
void UART1_ISR (void)
{
    u8 Local_i = 0;
    u8 sentbytes = 0;
    if ( GET_BIT( UARTRIS(UART1),TXRIS ) && ( UART_PrivInfoArr[UART_ChannelIndex1].UART_Channel_Status == UART_SUSPENDED_State ) )  /*The Transmitted FIFO level has passed through the condition defined in the UARTIFLS register */
    {
        while ( ( UART_PrivInfoArr[UART_ChannelIndex1].UART_SentDataLength < UART_PrivInfoArr[UART_ChannelIndex1].UART_DataLength ) && ( sentbytes <= ( UART_PrivInfoArr[UART_ChannelIndex1].TxFIFOTrigLength + 1 )  ) )
        {
            UARTDR(UART1) = UART_PrivInfoArr[UART_ChannelIndex1].ptrToBuffer[UART_PrivInfoArr[UART_ChannelIndex1].UART_SentDataLength] ;
            UART_PrivInfoArr[UART_ChannelIndex1].UART_SentDataLength++;
            sentbytes++;
        }
        if ( ( UART_PrivInfoArr[UART_ChannelIndex1].UART_SentDataLength )  == ( UART_PrivInfoArr[UART_ChannelIndex1].UART_DataLength ) )
        {
            UART_PrivInfoArr[UART_ChannelIndex1].UART_Channel_Status = UART_Job_Done;  /* means that the request is transfered to the UART ISR as we reached the FIFO trigger length */
            CLR_BIT(UARTIM( UART_PrivInfoArr[UART_ChannelIndex1].UART_NUM ) , TXIM );
        }
    }
    else if ( GET_BIT( UARTRIS(UART1),RXRIS ) && ( UART_PrivInfoArr[UART_ChannelIndex1].UART_Channel_Status == UART_SUSPENDED_State ) ) /*The received FIFO level has passed through the condition defined in the UARTIFLS register */
    {
        while( Local_i < ( UART_ConfigArray[UART_ChannelIndex2].RecieverFifo_Config.UART_RX_FIFO_TriggerLength + 1 ) )
        {
            if( !( GET_BIT( UARTFR(UART1) , RXFE ) ) ) /* first check that the FIFO is not empty */
            {
                if(  UART_PrivInfoArr[UART_ChannelIndex2].UART_ReceivedDatalength < UART_PrivInfoArr[UART_ChannelIndex2].UART_DataLength  )  /* then we check that the Requested data is still more than the received  */
                {
                    UART_PrivInfoArr[UART_ChannelIndex2].ptrToBuffer[UART_PrivInfoArr[UART_ChannelIndex2].UART_ReceivedDatalength]  = UARTDR(UART1) ;
                    UART_PrivInfoArr[UART_ChannelIndex2].UART_ReceivedDatalength++ ;
                    Local_i++;
                }
            }
        }
        if ( UART_PrivInfoArr[UART_ChannelIndex2].UART_ReceivedDatalength == UART_PrivInfoArr[UART_ChannelIndex2].UART_DataLength  )
        {
            UART_PrivInfoArr[UART_ChannelIndex2].UART_Channel_Status = UART_Job_Done;
            CLR_BIT(UARTIM( UART_PrivInfoArr[UART_ChannelIndex2].UART_NUM ) , RXIM );
        }
        RemainingReceiveBytes = UART_PrivInfoArr[UART_ChannelIndex2].UART_DataLength - UART_PrivInfoArr[UART_ChannelIndex2].UART_ReceivedDatalength ;
    }
}
...
void UART2_ISR (void)
{
    u8 Local_i = 0;
    u8 sentbytes = 0;
    if ( GET_BIT( UARTRIS(UART2),TXRIS ) && ( UART_PrivInfoArr[UART_ChannelIndex1].UART_Channel_Status == UART_SUSPENDED_State ) )  /*The Transmitted FIFO level has passed through the condition defined in the UARTIFLS register */
    {
        while ( ( UART_PrivInfoArr[UART_ChannelIndex1].UART_SentDataLength < UART_PrivInfoArr[UART_ChannelIndex1].UART_DataLength ) && ( sentbytes <= ( UART_PrivInfoArr[UART_ChannelIndex1].TxFIFOTrigLength + 1 )  ) )
        {
            UARTDR(UART2) = UART_PrivInfoArr[UART_ChannelIndex1].ptrToBuffer[UART_PrivInfoArr[UART_ChannelIndex1].UART_SentDataLength] ;
            UART_PrivInfoArr[UART_ChannelIndex1].UART_SentDataLength++;
            sentbytes++;
        }
        if ( ( UART_PrivInfoArr[UART_ChannelIndex1].UART_SentDataLength )  == ( UART_PrivInfoArr[UART_ChannelIndex1].UART_DataLength ) )
    ...
    else if ( GET_BIT( UARTRIS(UART2),RXRIS ) && ( UART_PrivInfoArr[UART_ChannelIndex2].UART_Channel_Status == UART_SUSPENDED_State ) ) /*The received FIFO level has passed through the condition defined in the UARTIFLS register */
    {
        while( !( GET_BIT( UARTFR(UART2) , RXFE ) ) && ( UART_PrivInfoArr[UART_ChannelIndex2].UART_ReceivedDatalength < UART_PrivInfoArr[UART_ChannelIndex2].UART_DataLength ) )
        {
            UART_PrivInfoArr[UART_ChannelIndex2].ptrToBuffer[UART_PrivInfoArr[UART_ChannelIndex2].UART_ReceivedDatalength]  = UARTDR(UART2) ;
            UART_PrivInfoArr[UART_ChannelIndex2].UART_ReceivedDatalength++ ;
            Local_i++;
        }
        if ( UART_PrivInfoArr[UART_ChannelIndex2].UART_ReceivedDatalength == UART_PrivInfoArr[UART_ChannelIndex2].UART_DataLength  )
